...
function initTanzaniaMap() {
  ...
  el._leafletMap = map; // share with initSightingHeatmap
  L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
    attribution: "&copy; OpenStreetMap contributors",
  }).addTo(map);
...
function initSightingHeatmap() {
  const el = document.querySelector("#tanzania-map");
  const map = el && el._leafletMap;
  ...
  function loadHeatmap() {
    ...
    if (species && species.value) params.set("bird_id", species.value);

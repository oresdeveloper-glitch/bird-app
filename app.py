...
from ai_engine import (
    ...
    localize_bird_regions,
    active_image_model_name,
)
...
def identify():
    ...
    if request.method == "POST":
        ...
        try:
            validate_and_compress_image(upload, output_path)
        ...
        if ref_emb and bird.id in ref_emb:
            ...
            gradcam_result = generate_gradcam_heatmap(
                output_path, gradcam_path, reference_embedding=ref_emb[bird.id]
            )
            if gradcam_result:
        ...
        yolo_result = localize_bird_regions(output_path)
        if yolo_result:
            detection_url = f"/static/uploads/{os.path.basename(yolo_result['overlay_path'])}"
            detection_results = yolo_result["detections"]
...
@app.route("/admin/confusion-matrix")
def admin_confusion_matrix():
    if not require_admin():
        flash("Admin access required.")
        return redirect(url_for("login"))

    # Serve precomputed JSON produced by confusion_matrix_eval.py.
    cm_path = os.path.join(PROJECT_ROOT, "confusion_matrix.json")
    top_path = os.path.join(PROJECT_ROOT, "confusion_matrix_top_misclassifications.json")

    error = None
    data = None
    if not os.path.exists(cm_path):
        error = (
            "confusion_matrix.json not found. Run: "
        ...
        try:
            ...
            if os.path.exists(top_path):
                with open(top_path, "r", encoding="utf-8") as f:
                    top_obj = json.load(f)
                    top_mis = top_obj.get("top_misclassifications", [])

            data = {
                "meta": cm_obj.get("meta", {}),
                "id_to_name": cm_obj.get("bird_id_to_name", {}),
                "top_mis": top_mis,
                "matrix_json": cm_obj.get("confusion_matrix", {}),
            }
        except Exception as exc:
            error = f"Failed to load confusion matrix JSON: {exc}"

    return render_template("confusion_matrix.html", error=error, data=data)
...
REFERENCE_EMBEDDINGS_VERSION = f"v1-{active_image_model_name()}"

print("  Initializing database and AI model (may take 30-60s first time)...")
import sys
sys.stdout.flush()
...
if __name__ == "__main__":
    ...
    app.run(host=host, port=port, debug=True, use_reloader=False, ssl_context=ssl_ctx)
